/**
 * AttendanceSummary — shows today's attendance stats from backend.
 * Displays present/absent counts and a mini progress bar.
 */
...
export default function AttendanceSummary({ refreshKey }) {
  const router = useRouter();
  const [data, setData] = useState(null);

  useEffect(() => {
    // Fetch today's attendance across all subjects
    api.get('/attendance', { params: { date: todayString() } })
      ...
      .catch(() => {
        // If query fails fall back to graceful null
        setData(null);
      });
  }, [refreshKey]);
