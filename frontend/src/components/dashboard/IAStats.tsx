  ...
  useEffect(() => {
    api.get('/iamarks')
      .then(res => {
        ...
        const avgs     = records.map((r: any) => (r.ia1 + r.ia2 + r.ia3) / 3);
        const classAvg = avgs.reduce((s: number, v: number) => s + v, 0) / avgs.length;
        const highest  = Math.max(...avgs);
        const lowest   = Math.min(...avgs);
        const passing  = avgs.filter((v: number) => v >= 10).length;
        setStats({ classAvg: classAvg.toFixed(1), highest: highest.toFixed(1), lowest: lowest.toFixed(1), passing, total: records.length });
      })
      .catch(() => {});
  }, [refreshKey]);
