...
export default function RecentActivity({ refreshKey }: RecentActivityProps) {
  const [activities, setActivities] = useState<ActivityItem[]>(FALLBACK);
  ...
  useEffect(() => {
    api.get('/dashboard/stats')
      .then(res => {
        ...
        if (raw?.length) {
          setActivities(raw.map((text: string) => ({ text, route: routeForActivity(text), icon: 'ellipse' })));
        }
      })
      .catch(() => {});
  }, [refreshKey]);
      ...
      {activities.map((item, idx) => (
        ...
        >
          <View style={styles.iconWrap}>
            <Ionicons name={(item.icon ?? 'ellipse') as any} size={14} color={Colors.primary} />
          </View>
