...
const TIMETABLE: Record<DayKey, Omit<ClassItem, 'status'>[]> = {
  Mon: [
...
export default function TodaysClasses() {
  ...
  const now       = new Date();
  const dayKey    = JS_DAY[now.getDay()];
  ...
  const classes = useMemo<ClassItem[]>(() => {
    ...
    return raw.map(c => {
      ...
      const status: ClassItem['status'] = nowHour > s + 1 ? 'done' : nowHour >= s ? 'now' : 'upcoming';
      return { ...c, status };
    });
  }, [dayKey, nowHour]);
