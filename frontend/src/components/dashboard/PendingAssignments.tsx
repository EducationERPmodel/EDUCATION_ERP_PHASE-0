    ...
    api.get('/assignments')
      .then(res => {
        const open = res.data
          ...
          .map((a: any) => ({ ...a, dueDate: a.dueDate || a.due_date }))
          .sort((a: any, b: any) => daysUntil(a.dueDate) - daysUntil(b.dueDate))
          .slice(0, 4);
        setAssignments(open);
      })
      .catch(() => {
        setAssignments((mockAssignments as any[]).filter((a: any) => a.status === 'Open').slice(0, 4));
      });
  }, [refreshKey]);
