...
async function pickFile(setFile: (f: PickedFile | null) => void): Promise<void> {
  try {
    const result = await DocumentPicker.getDocumentAsync({
      type: ['text/plain', 'application/pdf'],
      copyToCacheDirectory: true,
    });
    // expo-document-picker v14: result.canceled is a boolean, assets is the array
...
function FileSlot({ label, file, setFile }: FileSlotProps) {
  return (
...
const styles = StyleSheet.create({
  ...
  slot:         { marginBottom: 6 },
  slotLabel:    { fontSize: 13, fontWeight: '600', color: Colors.textSecondary, marginBottom: 8 },
  pickBtn:      { flexDirection: 'row', alignItems: 'center', gap: 8, borderWidth: 1.5, borderColor: Colors.primary, borderStyle: 'dashed', borderRadius: 10, padding: 12 },
  pickBtnText:  { color: Colors.primary, fontWeight: '600', fontSize: 14 },
  selectedRow:  { flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 6 },
  selectedText: { color: Colors.success, fontSize: 13, flex: 1 },
