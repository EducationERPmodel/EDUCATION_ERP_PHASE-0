...
export default function AttendanceScreen() {
  ...
  // ── Fetch attendance for selected subject + date ───────────────
  const fetchAttendance = useCallback(async (subj, dt) => {
    setLoading(true);
    try {
  ...
  const handleSubjectChange = (val) => { setSubject(val); fetchAttendance(val, date); };
  const handleDateChange    = (val) => { setDate(val);    fetchAttendance(subject, val); setShowDatePicker(false); };
          ...
          <ScrollView style={styles.calendarScroll} showsVerticalScrollIndicator={false}>
            {CALENDAR.map((item) => (
              <TouchableOpacity
                ...
                style={[
                  styles.calendarRow,
                  item.value === date && styles.calendarRowActive,
                ]}
              ...
              >
                ...
                <Text style={[
                  ...
                  item.value === date && styles.calendarLabelActive,
                ]}>
                  {item.label}
                </Text>
