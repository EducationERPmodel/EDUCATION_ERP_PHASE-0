 ...
 * Lecture Hall: L308  |  Class Strength: 66
 * Source: SVCE Class Timetable ACD06 (W.E.F. 27.04.2026)
 */
...
// ── Full SVCE timetable — Semester 4 BHASKARA ────────────────────
const TIMETABLE = {
  Mon: [
...
export default function TimetableScreen() {
  ...
  const teaching = periods.filter(p => !['Break','Lunch','NSS/PT','CCA/ECA','Skill/Counselling','Online Certification/Skill Enhancement'].includes(p.subject));
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.periodsContainer}>
          {periods.map((p, idx) => {
            const isBreakType = ['Break', 'Lunch'].includes(p.subject);
            if (isBreakType) {
              return (
            ...
            return (
              <View key={idx} style={[
                ...
                <View style={[styles.subjectPill, { backgroundColor: c.bg }]}>
                  <Text style={[styles.subjectText, { color: c.text }]} numberOfLines={1}>
                    {p.subject}
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  dayHeading: {
    ...
    paddingHorizontal: 16, marginTop: 14, gap: 8,
  },
  ...
  cardNow:     {
    borderWidth: 1, borderLeftWidth: 4, borderColor: Colors.primary + '55',
    elevation: 4, shadowOpacity: 0.12,
  },
