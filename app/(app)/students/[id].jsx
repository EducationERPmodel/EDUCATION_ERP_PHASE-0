...
const TABS = ['Overview','Attendance','IA Marks','Assignments','Achievements'];
const ACH_CATS = ['Certification','Hackathon','Event','Competition','Publication','Award','Other'];
const ACH_META = {
...
const EMPTY = { category:'Certification', title:'', issuer:'', date_achieved:'', description:'' };
export default function StudentProfileScreen() {
  ...
  const delAch = achId => Alert.alert('Delete','Remove this achievement?',[
  ...
  const { student, attendanceSummary, overallAttendance,
          recentAttendance, iaMarks, assignments, achievements = [] } = profile;
  ...
  return (
    <View style={s.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}
        ...
        <View style={s.hero}>
          <View style={s.avatar}>
            <Text style={s.avatarTxt}>{student.name.split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase()}</Text>
          </View>
          ...
          <View style={s.heroPills}>
            ...
            <HPill label="IA Avg"
              value={iaMarks.length?`${(iaMarks.reduce((t,r)=>t+Number(r.average),0)/iaMarks.length).toFixed(1)}`:'—'}
              color={Colors.purple} />
        ...
        <View style={s.body}>
          ...
          {tab==='Achievements' && <AchTab items={achievements}
        ...
        <View key={i} style={[s.infoRow, i%2===0 && s.infoAlt]}>
          <View style={s.infoL}>
            <View style={[s.infoIco, r.hi && { backgroundColor:Colors.purpleLight }]}>
              <Ionicons name={r.icon} size={13} color={r.hi ? Colors.purple : Colors.primary} />
            </View>
...
// ─── Attendance Tab ───────────────────────────────────────────────
function AttendanceTab({ summary, recent, overall }) {
  const bc = p => p>=75?Colors.success:p>=60?Colors.warning:Colors.danger;
  return (
    <View>
      ...
      {!summary.length
        ...
        : summary.map((item,i) => (
          <View key={i} style={s.attRow}>
            ...
            <View style={s.attBar}>
              <View style={s.attBg}>
                <View style={[s.attFill, { width:`${item.percent}%`, backgroundColor:bc(item.percent) }]} />
              </View>
...
// ─── IA Marks Tab ─────────────────────────────────────────────────
function IATab({ marks }) {
  if (!marks.length) return <Text style={s.noData}>No IA marks recorded.</Text>;
  const grade = avg => avg>=18?{l:'O',c:Colors.success}:avg>=15?{l:'A+',c:Colors.primary}:avg>=12?{l:'A',c:Colors.warning}:{l:'B',c:Colors.danger};
  return (
    <View>{marks.map((m,i) => {
      ...
      return (
        <View key={i} style={s.iaCard}>
          ...
          <View style={s.scores}>
            {[['IA1',m.ia1],['IA2',m.ia2],['IA3',m.ia3]].map(([l,v]) => (
...
// ─── Assignments Tab ──────────────────────────────────────────────
function AssignTab({ assignments }) {
...
// ─── Achievements Tab ─────────────────────────────────────────────
function AchTab({ items, onAdd, onEdit, onDelete }) {
  const grouped = ACH_CATS.reduce((acc,cat) => {
  ...
  return (
    <View>
      ...
      {!items.length ? (
        ...
        const m = ACH_META[cat] || ACH_META.Other;
        return (
...
// ─── Achievement Modal ────────────────────────────────────────────
function AchModal({ visible, onClose, onSaved, studentId, usn, edit }) {
  ...
  useEffect(() => {
    setForm(edit ? {
      category:     edit.category||'Certification',
      title:        edit.title||'',
      issuer:       edit.issuer||'',
        ...
        <View style={s.modalSheet}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight:420 }}>
            ...
            <View style={s.pickerBox}>
              <Picker selectedValue={form.category} onValueChange={set('category')} style={{ height:50 }}>
            ...
            {[
              ...
              <View key={f.k}>
                ...
                <TextInput
                  ...
                  value={form[f.k]} onChangeText={set(f.k)} multiline={!!f.multi}
                />
              </View>
            ))}
...
// ─── Small helpers ────────────────────────────────────────────────
function HBadge({ icon, label, color }) {
  return (
    <View style={s.hBadge}>
      <Ionicons name={icon} size={11} color={color||Colors.white} />
      <Text style={[s.hBadgeTxt, color&&{color}]} numberOfLines={1}>{label}</Text>
    </View>
  );
}
function HPill({ label, value, color }) {
  return (
