...
export default function AICheckerScreen() {
  ...
  const handleCheck = async () => {
    ...
    try {
      // Step 1 — extract Assignment 1
      setStatusMsg('Extracting text from Assignment 1...');
      let raw1;
      try {
        raw1 = await extractTextFromFile(file1);
      } catch (e) {
        Alert.alert('Extraction Failed', `Could not read "${file1.name}".\n${e.message}`);
        return;
      }

      // Step 2 — extract Assignment 2
      setStatusMsg('Extracting text from Assignment 2...');
      let raw2;
      try {
        raw2 = await extractTextFromFile(file2);
      } catch (e) {
        Alert.alert('Extraction Failed', `Could not read "${file2.name}".\n${e.message}`);
        return;
      }

      if (!raw1 || raw1.trim().length < 5) {
        Alert.alert('Empty File', `"${file1.name}" appears to be empty or unreadable.`);
        return;
      }
      if (!raw2 || raw2.trim().length < 5) {
        Alert.alert('Empty File', `"${file2.name}" appears to be empty or unreadable.`);
        return;
      }

      // Step 3 — clean and compare
